...
class _Front extends \IPS\Dispatcher\Standard
{
	...
	public function init()
	{
		...
		$legalControllers = array( 'privacy', 'contact', 'terms', 'embed', 'metatags', 'subscriptions' );
		...
		if( \IPS\Member::loggedIn()->member_id and $this->controller !== 'language' and $this->controller !== 'theme' and $this->controller !== 'ajax' and !\in_array( $this->controller, $legalControllers ) )
		{
			...
			elseif( \IPS\Member::loggedIn()->members_bitoptions['validating'] and $this->controller !== 'register' and $this->controller !== 'login' and $this->controller != 'redirect' and $this->controller !== 'store' and $this->controller !== 'checkout' )
			{
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=core&module=system&controller=register&do=validating', 'front', 'register' ) );
			}
...
Disallow: {$path}*?do=add
Disallow: {$path}*?do=submit
...
Disallow: {$path}*?tab=comments
Disallow: {$path}*?do=email
Disallow: {$path}*?do=findComment
Disallow: {$path}*?do=getLastComment
Disallow: {$path}*?do=getNewComment
Disallow: {$path}*?do=reportComment
Disallow: {$path}*?do=markRead
