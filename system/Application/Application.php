...
EOF;
	...
	public function build()
	{
		...
		/* Take care of features for this app */
		$this->buildNewFeatures();
		...
		\file_put_contents( $setupDir . '/data.json', json_encode( array(
			...
			'steps'					=> array(
				...
				'whatsnew'				=> file_exists( $setupDir . "/whatsnew.json" ),
	...
	/**
	 * Build New Feature Highlights
	 *
	 * @return	void
	 */
	public function buildNewFeatures()
	{
		/* Assign any that are flagged is "current" to this version */
		\IPS\Db::i()->update( 'core_members_features', array( 'feature_version_added' => $this->long_version ), array( "feature_version_added=? AND feature_app=?", -1, $this->directory ) );
		
		/* Write the JSON file */
		\IPS\Application\Feature::writeJsonFile( $this->directory, $this->long_version );
	}
	
	/**
	 * Install new feature highlights
	 *
	 * @param	string	$appKey		The application key
	 * @param	int		$versionId	The version ID
	 * @return	void
	 */
	public static function installNewFeatures( string $appKey, int $versionId )
	{
		...
		if ( file_exists( \IPS\ROOT_PATH . "/applications/{$appKey}/setup/upg_{$versionId}/whatsnew.json" ) )
		{
			if ( $data = @json_decode( \file_get_contents( \IPS\Application::load( $appKey )->getApplicationPath() . "/setup/upg_{$versionId}/whatsnew.json" ), TRUE ) )
			{
				\IPS\Db::i()->insert( 'core_members_features', $data );
			}
		}
	}
	...
	{		
		...
		if ( \IPS\CIC2 )
		{
			if ( !\in_array( $this->directory, \IPS\IPS::$ipsApps ) )
			{
				\IPS\Cicloud\folderDelete( "applications/{$this->directory}" );
			}
		}
		elseif ( !\IPS\IN_DEV AND !\IPS\DEMO_MODE AND file_exists( \IPS\ROOT_PATH . '/applications/' . $this->directory ) )
		{
			try
			{
