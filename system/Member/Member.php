...
class _Member extends \IPS\Patterns\ActiveRecord
{
	...
	public function getFeatureHighlights( bool $markRead = TRUE ): ?array
	{
		if ( $this->members_bitoptions['is_support_account'] )
		{
			/* Support account doesn't need these. */
			return NULL;
		}
		
		if ( !$this->members_bitoptions['has_new_acp_features'] )
		{
			/* No new features to see */
			return NULL;
		}
		
		/* Compile all unread features */
		if ( $this->_features === NULL )
		{
			$this->_features = array();
			foreach( new \IPS\Patterns\ActiveRecordIterator( \IPS\Db::i()->select( '*', 'core_members_features', array( \IPS\Db::i()->in( 'feature_id', $this->getFeatureReadIds(), true ) ) ), 'IPS\Application\Feature' ) AS $feature )
			{
				$this->_features[] = $feature;
			}
		}
		
		if ( \count( $this->_features ) )
		{
			/* Mark them as read */
			if ( $markRead === TRUE )
			{
				/* Only set the flag if we're actually marking them as read. */
				$this->members_bitoptions['has_new_acp_features'] = FALSE;
				$this->save();
				
				\IPS\Application\Feature::markRead( $this, ...$this->_features );
			}
			return $this->_features;
		}
		else
		{
			/* Even if we aren't marking them as read, there are none anyway, so just reset the flag. */
			$this->members_bitoptions['has_new_acp_features'] = FALSE;
			$this->save();
	...
	/**
	 * Get read feature ID's
	 *
	 * @return	array
	 */
	protected function getFeatureReadIds(): array
	{
		$return = array();
		foreach( \IPS\Db::i()->select( 'feature_id', 'core_members_feature_seen', array( "member_id=?", $this->member_id ) ) AS $id )
		{
			$return[] = $id;
		}
		return $return;
	...
	public function notificationsConfiguration()
	{
		if( $this->notificationsConfiguration === NULL )
		{
			...
			foreach (
				...
				{
					$this->notificationsConfiguration[ $row['notification_key'] ] = explode( ',', $row['default'] );
				}
				else
				{
			...
			if ( \IPS\Member::loggedIn()->canAccessModule( \IPS\Application\Module::get( 'core', 'modcp' ) ) )
			{
				if( !\IPS\Member::loggedIn()->members_bitoptions['no_report_count'] OR $force === TRUE )
				{
					...
					$reportCount = \IPS\Db::i()->select(
						'COUNT(*)',
						'core_rc_index',
						array( $where, \IPS\Db::i()->select( 'perm_id', 'core_permission_index', \IPS\Db::i()->findInSet( 'perm_view', $this->permissionArray() ) . " OR perm_view='*'" ) )
					)->first();
	...
	public function apiOutput( \IPS\Member $authorizedMember = NULL, $otherFields = NULL )
	{
		...
		$return['photoUrlIsDefault']	= $this->pp_photo_type == 'none' or static::photoUrl( $this->_data, FALSE, FALSE, FALSE ) != $return['photoUrl'];
	...
	public function daysSinceLastRank(): int
	{
		...
		$time = ( $lastRank ) ? $lastRank['time'] : $this->joined;
