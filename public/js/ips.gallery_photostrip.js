...
_photostrip.prototype.photostrip = {
	...
	display: function()
	{
		Debug.dir( ipb.photostrip._photoData );
		if ( $H( ipb.photostrip._photoData ) )
		{
			...
			{
				/* insert blanks */
				$('strip').insert( new Element( 'li', { 'id': 'strip_li_' + i } ).insert( new Element( 'img', { 'src': ipb.photostrip._stripless, 'class': 'galattach' } ) ) );
			...
			/* Observe if ye willl */
			$('photostripwrap').observe( 'mouseover', ipb.photostrip._showSliders );
			$('photostripwrap').observe( 'mouseout', ipb.photostrip._hideSliders );
	...
	_showSliders: function( e )
	{
		...
		$('slide_left').absolutize().setStyle( { 'top'    : ( _offset['top'] + 5 ) + 'px',
												 'left'   : ( _offset['left'] - 5 ) + 'px',
												 'display': 'block' } );
	
	
						
		$('slide_right').absolutize().setStyle( { 'top'    : ( _offset['top'] + 5 ) + 'px',
												  'left'   : ( ( _offset['left'] + _dims['width'] - 12 )  ) + 'px',
												  'display': 'block' } );
