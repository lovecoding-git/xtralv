...
_attach.prototype.attach = {
	...
	*/
	...
	{		
		...
		ipb.attach.optionsCache    = options;
		ipb.attach.optionsCache.id = id;
		
		Debug.write("Uploader registered");
	},
	...
	*/
	...
	_jsonPass: function( id, json )
	{
		...
		ipb.attach.uploaders[ ipb.attach.optionsCache.id ].json = json;
		ipb.attach.uploaders[ ipb.attach.optionsCache.id ].isReady();
...
_attach.prototype.attachSWF = Class.create({
	...
	*/
	...
	{
		temp = this.template.gsub(/\[id\]/, this.id + '_' + file.index).gsub(/\[name\]/, file.name.replace( /</g, '-' ).replace( />/g, '-' ) );
	...
	*/
	...
	{
		/* Mark W's fix for invisible header chars */
