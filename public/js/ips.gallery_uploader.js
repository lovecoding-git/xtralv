...
/* IPB3 Javascript								*/
/* -------------------------------------------- */
/* ipb.uploader.js - Attachment manager			*/
/* (c) IPS, Inc 2010							*/
/* -------------------------------------------- */
/* Author: Matt Mecham							*/
/************************************************/
...
_uploader.prototype.uploader = {
	...
	init: function()
	{
		Debug.write("Initializing ipb.uploader.js");
	...
	*/
	registerUploader: function( id, type, wrapper, options )
	{
		...
		{
			Debug.error("ipb.uploader.js: Attachment manager already has that ID");
			return;
		}
		// Already exists?
		if( ipb.uploader.uploaders[ id ] )
		{
			Debug.error("ipb.uploader.js: This uploader has already been registered");
		}
	...
	*/
	...
	_jsonPass: function( id, json )
	{
		...
		Debug.write( "ipb.uploader.js: Got json back from iframe id " + id );
	},
...
_uploader.prototype.attachTraditional = Class.create({
	...
	removeUpload: function( attachid, fileindex, fileinfo )
	{
		...
		Debug.write( "ipb.uploader.js: (ID " + this.id + ", removeUpload) Attach ID: " + attachid + ", File index: " + fileindex );
	},
	...
	*/
	isReady: function()
	{
		...
		Debug.write( "ipb.uploader.js: iFrame is ready" );
	},
...
_uploader.prototype.attachSWF = Class.create({
	...
	initialize: function( id, options, wrapper, template )
	{
		...
		try {
			...
			Debug.write( "ipb.uploader.js: (ID " + this.id + ") Created uploader");
			return true;
		}
		catch(e)
		{
			Debug.error( "ipb.uploader.js: (ID " + this.id + ") " + e );
			return false;
		}
	},
	...
	*/
	...
	{
		Debug.write( "ipb.uploader.js: (ID " + this.id + ", " + numFiles + " finished uploading");
	},
	...
	*/
	_fileQueueError: function( file, errorCode, message )
	{
		...
		try {
			...
			Debug.write( "ipb.uploader.js: (ID " + this.id + ", fileQueueError) " + errorCode + ": " + message );
		}
		catch( err )
		{
			Debug.write( "ipb.uploader.js: (ID " + this.id + ", fileQueueError) " + errorCode + ": " + message );
	...
	*/
	_uploadError: function( file, errorCode, message )
	{
		...
		Debug.write( "ipb.uploader.js: (ID " + this.id + ", uploadError) " + errorCode + ": " + message );
		return false;
	},
	...
	*/
	_uploadStart: function( file )
	{
		...
		Debug.write( "ipb.uploader.js: (ID " + this.id + ", uploadStart) " );
	},
	...
	*/
	_uploadSuccess: function( file, serverData )
	{
		...
		Debug.write( "ipb.uploader.js: (ID " + this.id + ", uploadSuccess) " + serverData );
	},
	...
	*/
	_uploadComplete: function( file )
	{
		...
		Debug.write( "ipb.uploader.js: (ID " + this.id + ", uploadComplete)" );
	},
	...
	*/
	_uploadProgress: function( file, bytesLoaded, bytesTotal)
	{
		...
		Debug.write( "ipb.uploader.js: (ID " + this.id + ", uploadProgress)" );
	},
	...
	*/
	...
	{ Debug.write( this.options.upload_url.replace( /album_id=(\d+?)?(&|$)/, 'album_id=' + ipb.uploader.getCurrentAlbumId() + '&' ) );
		...
		Debug.write( "ipb.uploader.js: (ID " + this.id + ", fileDialogComplete) Number: " + number + ", Queued: " + queued );
	...
	*/
	...
	{
		Debug.write( "ipb.uploader.js: Adding files" );
		this.obj.selectFile();
	}*/
});
