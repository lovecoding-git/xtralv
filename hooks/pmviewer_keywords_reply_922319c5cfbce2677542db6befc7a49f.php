...
/**
 * (SN) PM Viewer
 * PM Viewer Keyword Search Hook - Replies
		...
		while( $row = $this->DB->fetch( $o ) )
		{
			...
			while( $member = $this->DB->fetch( $b ) )
			{
				...
				IPSText::getTextClass( 'email' )->buildMessage( array(
																		...
																		'TEXT'		=> $this->settings['pmviewer_keyword_search_link'] ? $this->lang->words['pm_keyword_link_text'].' '.$this->settings['_admin_link'].'?&amp;app=pmviewer&amp;module=main&amp;section=overview&amp;do=viewTopic&amp;id='.$message['msg_topic_id'] : $this->lang->words['pm_keyword_nolink_text'].' '.$message['msg_topic_id'],
				...
				IPSText::getTextClass('email')->subject	= sprintf( 
																	...
																	( $this->settings['pmviewer_keyword_search_link'] ? '<a href="'.$this->settings['_admin_link'].'?&amp;app=pmviewer&amp;module=main&amp;section=overview&amp;do=viewTopic&amp;id='.$message['msg_topic_id'].'" target="_blank" title="'.$topic['mt_title'].'">'.$this->lang->words['pm_keyword_nolink_msg'].'</a>' : $this->lang->words['pm_keyword_nolink_msg'] )
																);
																
				$url = ( $this->settings['pmviewer_keyword_search_link'] == 1 ) ? $this->settings['_admin_link'].'?&amp;app=pmviewer&amp;module=main&amp;section=overview&amp;do=viewTopic&amp;id='.$message['msg_topic_id'] : '' ;
