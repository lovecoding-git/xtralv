...
class _view extends \IPS\Dispatcher\Controller
{
	...
	protected function _viewMonth( $jump, $featured=array() )
	{
		...
		try
		{
			...
			$showBirthday = FALSE;
			/* Show birthdays if we're on the calendar landing page */
			if ( $this->_calendar === NULL OR \count( $this->roots ) == 0 )
			{
				$showBirthday = TRUE;
			}
			/* show birthdays if there's only one calendar */
			if ( \IPS\Request::i()->id AND $this->_calendar !== NULL OR \count( $this->roots ) == 1 )
			{
				$showBirthday = TRUE;
			}
			/* Get birthdays */
			$birthdays	= $showBirthday ? $date->getBirthdays( FALSE, TRUE ) : array();
		...
		/* Display */
		$output = \IPS\Theme::i()->getTemplate( 'browse' )->calendarMonth( $this->roots, $date, $featured, $birthdays, $events, $this->_today, $this->_calendar, $jump );
	...
	protected function _viewWeek( $jump, $featured=array() )
	{
		...
		try
		{
			...
			/* Get birthdays */
			$birthdays	= ( $this->_calendar === NULL OR \count( $this->roots ) == 0 ) ? $date->getBirthdays( FALSE, TRUE ) : array();
		...
		/* Display */
		$output = \IPS\Theme::i()->getTemplate( 'browse' )->calendarWeek( $this->roots, $date, $featured, $birthdays, $events, $nextWeek, $lastWeek, $days, $this->_today, $this->_calendar, $jump );
	...
	protected function _viewDay( $jump, $featured=array() )
	{
		...
		$birthdayCount	= ( $this->_calendar === NULL OR \count( $this->roots ) == 0 ) ? $date->getBirthdays( TRUE, TRUE ) : array();
		$birthdays		= ( $this->_calendar === NULL OR \count( $this->roots ) == 0 ) ? $date->getBirthdays( TRUE, FALSE ) : array();
			...
			foreach( $_events as $type => $event )
			{
				foreach( $event as $_event )
				{
					...
					if( $_event->all_day AND (
						$_event->nextOccurrence( $date, 'startDate' )->strFormat('%d') == \IPS\Request::i()->d OR 
						$_event->nextOccurrence( $date, 'endDate' ) AND $_event->nextOccurrence( $date, 'endDate' )->strFormat('%d') == \IPS\Request::i()->d
					) )
					{
					...
					{
						if( $_event->nextOccurrence( $date, 'startDate' )->strFormat('%d') == \IPS\Request::i()->d )
						{
							$dayEvents[ $_event->_start_date->hours ][ $_event->id ]	= $_event;
						}
						elseif( $_event->nextOccurrence( $date, 'endDate' ) AND $_event->nextOccurrence( $date, 'endDate' )->strFormat('%d') == \IPS\Request::i()->d )
						{
		...
		/* Display */
		$output = \IPS\Theme::i()->getTemplate( 'browse' )->calendarDay( $this->roots, $date, $featured, $birthdays, $birthdayCount, $dayEvents, $tomorrow, $yesterday, $this->_today, $this->_calendar, $jump );
	...
	protected function _viewStream( $jump, $featured=array() )
	{
		...
		/* Display */
		$output = \IPS\Theme::i()->getTemplate( 'browse' )->calendarStream( $this->roots, $date, $featured, $events, $this->_calendar, $jump, $pagination );
