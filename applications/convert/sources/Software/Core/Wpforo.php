	...
	public function convertMembers()
	{
		...
		foreach( $this->fetch( 'users', 'users.ID' )->join( 'wpforo_profiles', "users.ID=wpforo_profiles.userid" ) AS $user )
		{
			...
			$info = [
				...
				'mgroup_others'				=> $user['secondary_groups'],
				'name'						=> $user['username'],
				...
				'last_visit'			    => new \IPS\DateTime( $user['last_login'] ),
				'last_activity'			    => new \IPS\DateTime( $user['last_login'] ),
			];
			...
			else
			{
				...
				{
					$filename = \strtolower( \str_replace( '_', '', \preg_replace( '/\s|\.(?=.*\.)/i', '-',$user['username'] ) ) ) . '_' . $user['ID'] . '.' . $ext;
