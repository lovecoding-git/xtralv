...
class _Rank extends \IPS\Node\Model
{
	...
	protected function get__icon()
	{
		if ( $this->icon )
		{
			return \IPS\File::get( 'core_Badges', $this->icon );
		}
		else
		{
	...
	public function form( &$form )
	{
		...
		$form->add( new \IPS\Helpers\Form\Upload( 'member_ranks_icon', $this->icon ? \IPS\File::get( 'core_Badges', $this->icon ) : NULL, TRUE, array( 'obscure' => TRUE, 'allowedFileTypes' => array_merge( \IPS\Image::supportedExtensions(), ['svg'] ), 'checkImage' => TRUE, 'storageExtension' => 'core_Badges' ), function( $val ) {
			if( !$val )
			{
	...
	public function delete()
	{
		if ( $this->icon )
		{
			try
			{
				\IPS\File::get( 'core_Badges', $this->icon )->delete();
			}
			catch( \Exception $ex ) { }
		}
	...
	public static function importXml( $file, $option=NULL )
	{
		...
		while ( $xml->read() and $xml->name == 'rank' )
		{
			...
			{
				$insert['icon'] = (string) \IPS\File::create( 'core_Badges', $insert['icon_name'], $insert['icon_data'], NULL, TRUE, NULL, FALSE );
