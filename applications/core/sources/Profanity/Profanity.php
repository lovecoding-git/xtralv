...
class _Profanity extends \IPS\Patterns\ActiveRecord implements \JsonSerializable
{
		...
		$table->parsers = array(
			'action'				=> function( $val, $row )
			{
				...
				else
				{
					if ( $row['min_posts'] )
					{
						return \IPS\Member::loggedIn()->language()->addToStack( 'profanity_filter_action_moderate_min_posts', NULL, array( 'pluralize' => array( $row['min_posts'] ) ) );
					}
	 ...
	 * @return	bool
	 */
	public static function hiddenByFilters( $content, $member=NULL )
	{
