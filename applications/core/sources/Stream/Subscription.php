...
class _Subscription extends \IPS\patterns\ActiveRecord
{
	 ...
	 * @return    bool
	 */
	public static function sendBatch( $frequency = 'daily', $numberToSend = 50 )
	{
		$subscriptions = iterator_to_array(
			\IPS\Db::i()->select( '*, core_members.last_visit', 'core_stream_subscriptions', array('frequency = ? AND sent < ? and last_visit > ?', $frequency, ( $frequency == 'daily' ) ? time() - 86400 : time() - 604800, time() - \IPS\Settings::i()->activity_stream_subscriptions_inactive_limit * 86400), 'sent ASC', array(0, $numberToSend), NULL, NULL, \IPS\Db::SELECT_DISTINCT )
	...
	public function getContentForStream( array $subscriptionRow ) : array
	{
		...
		$query->filterByCreateDate( \IPS\DateTime::ts( $this->sent ) );
