	...
	protected function edit()
	{
		...
		foreach ( \IPS\Notification::availableOptions( NULL, $extension ) as $key => $option )
		{
			if ( $option['type'] === 'standard' )
			{
				...
				if ( !\in_array( 'inline', $option['disabled'] ) or ( ( \IPS\Settings::i()->mobile_app_id || \IPS\Notification::webPushEnabled() ) and !\in_array( 'push', $option['disabled'] ) ) or !\in_array( 'email', $option['disabled'] ) )
				{
					...
					{		
						...
						{							
							if ( !\in_array( $k, $option['disabled'] ) )
							{
								if ( $k === 'push' )
								{
									if ( !\IPS\Settings::i()->mobile_app_id && !\IPS\Notification::webPushEnabled() )
									{
										continue;
									}
