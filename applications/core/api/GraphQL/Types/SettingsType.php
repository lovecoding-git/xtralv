				...
				return [
					...
					'mobileHomeBlocks' => [
						...
						'resolve' => function () {
							try 
							{
								$parsed = json_decode( \IPS\Settings::i()->mobile_app_home_blocks_order, TRUE );
								$blocksToShow = array_filter( $parsed );

								return array_keys( $blocksToShow );
							}
							catch (\Exception $e) 
							{
								return array("new_content", "active_users", "our_picks", "popular_contributors");
							}
						}
					],
					'version' => [
