...
class _topic extends \IPS\Content\Controller
{
	...
	protected function manage()
	{
		...
		/* If there's only one forum and we're not in a club, we actually don't want the nav */
		if ( !$topic->container()->club() AND ( \IPS\forums\Forum::theOnlyForum() or \IPS\forums\Forum::isSimpleView() ) )
		{
				...
				try
				{
					...
					if ( $truncatedAnswer = $answer->truncated( TRUE, NULL ) )
					{
						\IPS\Output::i()->jsonLd['topic']['mainEntity']['acceptedAnswer'] = array(
							...
							'upvoteCount' => $answer->reactionCount(),
							'author'	=> array(
								'@type'		=> 'Person',
			...
			foreach( $comments as $comment )
			{
				...
				if ( $truncatedComment = $comment->truncated( TRUE, NULL ) )
				{
					...
					$commentJson[$i] = array(
						...
						'upvoteCount' => $comment->reactionCount(),
		...
		if( $topic->isArchived() AND !\IPS\Member::loggedIn()->member_id )
		{
			...
			$httpHeaders = array( 'Expires'		=> \IPS\DateTime::create()->add( new \DateInterval( 'PT12H' ) )->rfc1123() ,
								  'Cache-Control'	=> "max-age=" . ( 60 * 60 * 12 ) . ", s-maxage=" . ( 60 * 60 * 12 ) . ", public, stale-if-error, stale-while-revalidate" );
