...
class _Forum extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	...
	public function formatFormValues( $values )
	{
		...
		/* Bitwise */
		foreach ( array( 'bw_disable_tagging', 'bw_disable_prefixes', 'bw_enable_answers_member', 'bw_enable_answers_moderator' ) as $k )
		{
			if( isset( $values[ $k ] ) )
			{
				$values['forums_bitoptions'][ $k ] = ( $k == 'bw_enable_answers_member' or $k == 'bw_enable_answers_moderator' ) ? $values[ $k ] : !$values[ $k ];
				unset( $values[ $k ] );
			}
		}
		...
		else
		{
			foreach ( \IPS\Db::i()->select( '*', 'core_moderators' ) as $moderator )
			{
				...
				if ( $moderator['perms'] === '*' OR
					(
						!empty( $json['can_read_all_topics'] ) AND ( isset( $json['forums'] ) AND ( $json['forums'] === -1 OR \in_array( $this->_id, $json['forums'] ) ) )
					) )
				{
					if( $moderator['type'] === 'g' )
					{
	...
	public function clubForm( \IPS\Helpers\Form $form, \IPS\Member\Club $club )
	{
		...
		if( $club->type == 'closed' )
		{
			$form->add( new \IPS\Helpers\Form\YesNo( 'club_node_public', $this->id ? $this->isPublic() : FALSE ) );
		}
	}
