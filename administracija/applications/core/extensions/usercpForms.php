 ...
 * Last Updated: $Date: 2015-05-03 10:31:01 -0400 (Sun, 03 May 2015) $
 * </pre>
 *
 * @author 		$Author: stuarts $
...
class usercpForms_core extends public_core_usercp_manualResolver implements interface_usercp
{
	...
	public function customEvent_updateAttachments()
	{
		...
		$this->request['secure_key'] = $this->request['secure_key'] ? $this->request['secure_key'] : $this->request['authKey'];

		if( $this->request['secure_key'] != $this->member->form_hash )
		{
			$this->registry->output->showError( 'warn_bad_key', '1021523.1' );
		}
	...
	public function customEvent_removeNotifications()
	{
		...
		if( $this->request['secure_key'] != $this->member->form_hash )
		{
			$this->registry->output->showError( 'warn_bad_key', 1021523 );
		}
	...
	public function customEvent_markNotification()
	{
		...
		{
			$this->registry->getClass('output')->showError( 'no_permission', 1021523 );
		}
	...
	public function customEvent_toggleIgnoredUser()
	{
		...
		if( $this->request['secure_key'] != $this->member->form_hash )
		{
			$this->registry->output->showError( 'warn_bad_key', '1021523.2' );
		}
	...
	public function customEvent_removeIgnoredUser()
	{
		...
		if( $this->request['secure_key'] != $this->member->form_hash )
		{
			$this->registry->output->showError( 'warn_bad_key', '1021523.3' );
		}
	...
	public function formFacebook( $inlineMsg='' )
	{
		...
		else
		{
			...
			/* Publish Stream */
			$perms['publish_actions'] = $facebook->fetchHasAppPermission( 'publish_actions' );
			
			/* Read stream */
			$perms['user_status']    = $facebook->fetchHasAppPermission( 'user_status' );
			
			//var_dump( $perms ); exit;
			
			/* Check our perms and see if any aren't allowed */
			$missingPerms = FALSE;
			foreach( $perms AS $k => $perm )
			{
				if ( $perm['data'][0]['status'] != 'granted' )
				{
					$missingPerms = TRUE;
					break;
				}
		...
		return $this->registry->getClass('output')->getTemplate('ucp')->membersFacebookConnect( trim($this->memberData['fb_uid']), $userData, $linkedMemberData, $perms, $missingPerms );
	}
	...
	public function formIgnoredUsers()
	{
		...
		while( $row = $this->DB->fetch() )
		{
 		...
 		$result = $this->registry->getClass('output')->getTemplate('ucp')->membersIgnoredUsersForm( $final_users, $pagination );
 		$result = str_replace( "area=toggleIgnoredUser", "area=toggleIgnoredUser&amp;authKey=" . $this->member->form_hash, $result );
 		$result = str_replace( "area=removeIgnoredUser", "area=removeIgnoredUser&amp;authKey=" . $this->member->form_hash, $result );
 		return $result;
	}
	...
	public function showInlineNotifications( $error='' )
	{
		...
		if ( isset($this->request['clear']) && trim($this->request['clear']) == 'true' AND $this->request['k'] == $this->member->form_hash )
		{
	...
	public function saveProfileInfo()
	{
		...
		if( $website_field && $fields->out_fields[ 'field_' . $website_field ] )
		{
			...
			if ( IPSText::xssCheckUrl( $fields->out_fields[ 'field_' . $website_field ] ) === FALSE )
			{
				$fields->error_fields['invalid'][] = $fields->cache_data[ $website_field ];
				$fields->out_fields[ 'field_' . $website_field ] = '';
			}
		}
		...
		/* http://community.invisionpower.com/resources/bugs.html/_/ip-board/cannot-uncheck-my-timezone-is-currently-in-dst-checkbox-in-profile-r41510 */
