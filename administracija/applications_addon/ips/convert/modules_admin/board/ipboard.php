		...
		private function convert_members()
		{
			...
			$main = array(	'select' 	=> 'm.*',
							...
							'add_join'	=> array(
											...
											array(	'select' => 'c.*',
													'from'	 => array( 'pfields_content' => 'c' ),
													'where'	 => 'm.member_id=c.member_id',
													'type'   => 'left'
												),
											array(	'select' => 'r.*',
													'from'	 => array( 'rc_modpref' => 'r' ),
													'where'	 => 'm.member_id=r.mem_id',
													'type'   => 'left'
												),
											),
						);
			...
			{
				foreach ( $row as $field => $value ) if ( stristr('tc_', $field) ) unset($row[$field]);
				...
				{
					if (in_array($key, array('notes', 'links', 'bio', 'ta_size', 'signature', 'avatar_location', 'avatar_size', 'avatar_type', 'pconversation_filters', 'fb_photo', 'fb_photo_thumb', 'fb_bwoptions')) or preg_match('/pp_(.+?)/', $key))
					{
						$profile[$key] = $row[$key];
					}
					elseif ($key == 'updated' or preg_match('/field_(.+?)/', $key))
					{
						$custom[$key] = $row[$key];
					}
					elseif (in_array($key, array('mem_id', 'by_pm', 'by_email', 'by_alert', 'rss_key', 'max_points', 'reports_pp', 'rss_cache')))
					{
						$reportpref[$key] = $row[$key];
					}
					else
					{
						$member[$key] = $row[$key];
				...
				// Reparse siggy, about me, etc.
				$profile['pp_bio_content'] = $this->fixPostData($profile['pp_bio_content']);
				$profile['pp_about_me'] = $this->fixPostData($profile['pp_about_me']);
				$profile['notes'] = $this->fixPostData($profile['notes']);
				$profile['bio'] = $this->fixPostData($profile['bio']);
				$profile['signature'] = $this->fixPostData($profile['signature']);
				
				// And send it to the lib								
				$this->lib->convertMember($info, $member, $profile, $custom, $us['pp_path'], $us['pp_path']);
				...
				if ($reportpref['mem_id'])
				{
					$reportpref['mem_id'] = $this->lib->getLink($reportpref['mem_id'], 'members');
					$this->DB->insert( 'rc_modpref', $reportpref );
				}
