	    ...
	    */
		...
		public function countRows($action)
		{
			switch ($action)
			{
				...
				case 'ranks':
					return count($this->_fetchUserTitles());
					break;
		...
		private function convert_pms()
		{
			...
			foreach ( $memberList as $memberId )
			{
				...
				foreach ( $memberPms as $memberPm )
				{
					...
					if ( $memberPm[0] == 1 ) {
						$starterId = $this->lib->getLink($memberPm[3], 'members');
						$recipientId = $this->lib->getLink($memberId, 'members');
					} else {
						$starterId = $this->lib->getLink($memberId, 'members');
						$recipientId = $this->lib->getLink($memberPm[3], 'members');
					}
