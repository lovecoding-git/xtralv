 ...
 */
	...
	$info = array(
		'key'	=> 'vbulletin_legacy36',
		'name'	=> 'vBulletin 3.6',
		'login'	=> true,
	);

	class admin_convert_board_vbulletin_legacy36 extends ipsCommand
	{
		/**
	    ...
	    */
		...
		private function convert_topics()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$save = array(
					'title'			  	=> ($prefix) ? $prefix['text'].' '.$row['title'] : $row['title'],
		...
		private function convert_posts()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				ipsRegistry::DB('hb')->execute();
				while ($reprow = ipsRegistry::DB('hb')->fetch())
				{
		...
		private function convert_polls()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$poll_array = array(
					// vB only allows one question per poll
					1 => array(
