	    ...
	    */
	    public function doExecute( ipsRegistry $registry )
		{
			...
			$this->actions = array(
				...
				'gallery_images'		=> array('members', 'gallery_categories'),
			...
			$this->lib =  new convertGallery( $this->registry, $html, $this, false );
	    ...
	    */
		...
		public function countRows($action)
		{
			switch ($action)
			{
				...
				case 'gallery_ecardlog':
					return $this->lib->countRows('postcards');
					break;
		...
		private function convert_gallery_categories()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				$save = array(
					...
					'c_order'			=> $row['cat_order'],
					);

				$this->lib->convertCategory($row['cat_id'], $save, array());
			}
		...
		private function convert_gallery_images()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				// Go!
				$this->lib->convertImage($row['image_id'], $save, $path, array());
		...
		private function convert_gallery_ecardlog()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				$user = ipsRegistry::DB('hb')->buildAndFetch( array( 'select' => '*', 'from' => 'users', 'where' => "user_email='{$row['postcard_sender_email']}'" ) );

				$save = array(
					'img_id'		=> $row['image_id'],
					'edate'			=> $row['postcard_date'],
					'member_id'		=> $user['user_id'],
					'receiver_name'	=> $row['postcard_recipient_name'],
					'receiver_email'=> $row['postcard_recipient_email'],
					'title'			=> $row['postcard_headline'],
					'msg'			=> $row['postcard_message'],
					'bg'			=> $row['postcard_bg_color'],
					'font'			=> $row['postcard_font_color'],
					'border'		=> $row['postcard_border_color'],
					);

				$this->lib->convertECard($row['postcard_id'], $save);
			}
