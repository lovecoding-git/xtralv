	...
	class admin_convert_gallery_smfmediagallery extends ipsCommand
	{
	    ...
	    */
		...
		private function convert_gallery_albums()
		{
			...
			if(!$this->request['st'])
			{
				$value = array('gallery_categories', 'conv_parent', 'mediumint(5)');
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				if($row['type'] == 'user')
				{
					$save = array(
						...
						'images'		=> $row['num_items'],
						'member_id'		=> $row['album_of'],
						'category_id'	=> $us['orphans'],
						);

					$this->lib->convertAlbum($row['id_album'], $save, true);
				}
				else
				{
					...
					$this->lib->convertCategory($row['id_album'], $save, array());
				}
		...
		private function convert_gallery_images()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$cat_link = $this->lib->getLink($row['album_id'], 'gallery_categories', true);
				...
				$save = array(
					...
					'category_id'	=> ($cat_link) ? $row['album_id'] : 0,
					'album_id'		=> ($album_link) ? $row['album_id'] : 0,
					'caption'		=> $row['title'],
				...
				$this->lib->convertImage($row['id_media'], $save, $path, array());
