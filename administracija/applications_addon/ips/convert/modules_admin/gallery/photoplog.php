	...
	class admin_convert_gallery_photoplog extends ipsCommand
	{
	    ...
	    */
	    public function doExecute( ipsRegistry $registry )
		{
			...
			$forAll = array( 'gallery_categories'	=> array('members'),
	    ...
	    */
		...
		private function convert_gallery_categories()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$save = array(
					'parent'			=> $row['parentid'] == -1 ? 0 : $row['parentid'],
					'name'				=> $row['title'],
					'description'		=> $row['description'],
					'c_order'			=> $row['displayorder'],
					'category_only' => $row['parentid'] == -1 ? 1 : 0,
					'album_mode' => 0,
					//'images'			=> $row['photos'],
					//'comments'			=> $row['posts'],
					//'password'			=> $row['password'],
					//'cat_rule_method'	=> ($row['intro'] == 'yes') ? 2 : 0,
					//'cat_rule_title'	=> $row['introtitle'],
					//'cat_rule_text'		=> $row['introcopy'],
					);
		...
		private function convert_gallery_images()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				// Go!
				$this->lib->convertImage($row['fileid'], $save, $path, array());
