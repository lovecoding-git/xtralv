		...
		private function convert_gallery_albums()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				$save = array(
					...
					'public_album'		=> ($row['private'] == 'yes') ? 0 : 1,
					'name'				=> $row['catname'],
					'images'			=> $row['photos'],
					'comments'			=> $row['posts'],
					'name'				=> $row['catname'],
				...
				$this->lib->convertAlbum($row['id'], $save, true);
			}
		...
		private function convert_gallery_images()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$save = array(
					...
					'category_id'		=> ( $this->lib->getLink($row['cat'], 'gallery_categories', true) ) ? $row['cat'] : 0,
				...
				// Go!
				$this->lib->convertImage($row['id'], $save, $path, array());
				...
				{
					$this->lib->convertFav( $fav['id'], array( 'img_id' => $fav['photo'], 'member_id' => $row['userid'] ) );
				}
