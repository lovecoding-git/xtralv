 ...
 */
	...
	$info = array(
		'key'	=> 'coppermine',
		'name'	=> 'Coppermine Photo Gallery 1.4',
		'login'	=> false,
	);
	    ...
	    */
		...
		private function convert_gallery_categories()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$this->lib->convertCategory($row['cid'], $save, array());
			}
		...
		private function convert_gallery_albums()
		{
			...
			$this->lib->getMoreInfo('gallery_albums', $loop, array('orphans' => array('type' => 'dropdown', 'label' => 'To which category do you wish to put memers albums, or albums with no category?', 'options' => $cats)));
			...
			{
				$skip_cat_link = false;
				if (!$this->lib->getLink($row['category'], 'gallery_categories', true))
				{
					$skip_cat_link = true;
				...
				$save = array(
					...
					'category_id'		=> ($skip_cat_link) ? $us['orphans'] : $row['category'],
					'member_id'			=> $member_id,
					);

				$this->lib->convertAlbum($row['aid'], $save, $skip_cat_link);
			}
		...
		private function convert_gallery_images()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				// Go!
				$this->lib->convertImage($row['pid'], $save, $path, $custom);
