 ...
 * Photopost
 * Last Update: $Date: 2009-06-10 15:19:51 +0100 (Wed, 10 Jun 2009) $
...
{		
	...
	*/
	public function doExecute( ipsRegistry $registry )
	{
		...
		$this->lib =  new convertGallery( $this->registry, $html, $this, $useLocal );
	...
	*/
	...
	/*	$groups = array();
		...
		while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
		{
			...
			$save = array(
				...
				'c_order'			=> $row['displayorder'],
				'images'			=> $row['imagecount'],
				'comments'			=> $row['postcount'],
				'password'			=> $row['password'],
				);
			
			$this->lib->convertCategory($row['catid'], $save, $perms);			
		}
	...
	private function convert_gallery_albums()
	{
		...
		{
			$save = array(
				'member_id'			=> $row['parent'],
				'public_album'		=> 1,
				'name'				=> $row['title'],
				'images'			=> $row['imagecount'],
				'comments'			=> $row['postcount'],
				'category_id'		=> $row['parent'] == 0 ? $us['orphans'] : $row['parent'],
				);
			
			$this->lib->convertAlbum($row['catid'], $save, true);			
		}
	...
	private function convert_gallery_images()
	{
		...
		while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
		{
			...
			$save = array( 'member_id'			=> $row['userid'],
						   'category_id'		=> ( $this->lib->getLink($row['catid'], 'gallery_categories', true) ) ? $row['catid'] : 0,
						   'album_id'			=> ( $this->lib->getLink($row['catid'], 'gallery_albums', true) ) ? $row['catid'] : 0,
						   'caption'			=> $row['title'],
						   'description'		=> $row['description'],
						   'directory'			=> $row['catid'],
						   'file_name'			=> $row['filename'],
						   'file_size'			=> $row['originalfilesize'],
						   'file_type'			=> $mime,
						   'approved'			=> $row['open'],
						   'views'		   => $row['views'],
						   'comments'	   => $row['posts'],
			...
			// Go!				
			$this->lib->convertImage($row['imageid'], $save, $path . $tmpPath, array());
			
			//-----------------------------------------
			// Favourites
			//-----------------------------------------
			$favs = array(	'select' 	=> '*',
							'from' 		=> 'ppgal_favorites',
							'order'		=> 'imageid ASC',
			...
			{
				$this->lib->convertFav( $fav['imageid'].$fav['userid'], array( 'img_id' => $fav['imageid'], 'member_id' => $row['userid'] ) );
			}
