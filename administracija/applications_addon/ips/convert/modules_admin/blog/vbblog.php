 ...
 */
	...
	class admin_convert_blog_vbblog extends ipsCommand
	{
	    ...
	    */
	    public function doExecute( ipsRegistry $registry )
		{
			...
			$this->lib->sendHeader( 'vBulletin Blog &rarr; IP.Blog Converter' );
		...
		public function countRows($action)
		{
			switch ($action)
			{
				...
				case 'blog_attachments':
					return $this->lib->countRows('blog_attachment');
					break;
		...
		private function convert_blog_attachments()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				if ($row['filedata'])
				{
					...
					$done = $this->lib->convertAttachment($row['attachmentid'], $save, '', true, true);
				}
				...
				else
				{
					...
					$done = $this->lib->convertAttachment($row['attachmentid'], $save, $path, false, true);
				}
