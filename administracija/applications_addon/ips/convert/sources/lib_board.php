	...
	class lib_board extends lib_master
	{
		...
		public function getInfo()
		{
			return "<strong>Rebuild Content</strong><br />
				<a href='{$this->settings['base_url']}&app=core&module=tools&section=rebuild&do=rebuild_overview' target='_blank'>Click here</a> and run the following tools:
				<ul>
		...
		public function convertBBCode($id, $info, $dupes)
		{
			...
			$dupe = $this->DB->buildAndFetch( array( 'select' => 'bbcode_id', 'from' => 'custom_bbcode', 'where' => "bbcode_tag = '{$info['bbcode_tag']}'" ) );
			if ($dupe)
			{
				if ($dupes == 'local')
				{
				...
				{
					$this->DB->delete('custom_bbcode', "bbcode_id={$dupe['bbcode_id']}");
				}
			}
