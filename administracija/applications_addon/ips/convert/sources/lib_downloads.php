 ...
 */
	class lib_downloads extends lib_master
	{
		...
		public function truncate($action)
		{
			switch ($action)
			{
				...
				case 'downloads_files_records':
					return array( 'downloads_files_records' => 'record_id ' );
					break;
		...
		{	
			...
			{
				$this->logError($id, 'Type already exists: '.$info['mime_mimetype']);
				return false;
			}
		 ...
		 * @return 	boolean		Success or fail
		 **/
		public function convertFile($id, $info, $cfields)
		{	
			//-----------------------------------------
			// Make sure we have everything we need
			//-----------------------------------------
			...
			if (!$id)
			{
				$this->logError($id, '(FILE) ID number provided');
				return false;
			}
			if (!$info['file_name'])
			{
				$this->logError($id, 'No FILE name provided');
				return false;
			}
			if (!$info['file_cat'])
			{
				$this->logError($id, 'No FILE category provided');
				return false;
			}
			/*if (!$info['file_filename'])
			{
				$this->logError($id, 'No FILE filename provided');
				return false;
			}*/
		 ...
		 * @return 	boolean		Success or fail
		 **/
		public function convertFileRecord($id, $info, $options)
		{	
			//-----------------------------------------
			// Make sure we have everything we need
			//-----------------------------------------
			...
			if (!$id)
			{
				$this->logError($id, '(RECORD) No ID number provided');
				return false;
			}
			if (!$info['record_file_id'])
			{
				$this->logError($id, 'No RECORD File ID provided');
				return false;
			}
			if (!$info['record_type'])
			{
				$this->logError($id, 'No RECORD type provided');
				return false;
			}
			if (!$info['record_location'])
			{
				$this->logError($id, 'No RECORD location provided');
				return false;
			}
			if (!$info['record_storagetype'])
			{
			...
			if ($info['record_storagetype'] == 'web' or $info['record_storagetype'] == 'nonweb')
			{
				...
				if ( in_array($info['record_type'], array('ssupload','sslink')) )
				{
					$this->moveFiles(array($info['record_location']), $options['remote_ss_path'], $sssave);
				}
				else
				{
					$this->moveFiles(array($info['record_location']), $options['remote_path'], $save);
				}
			}
			elseif ($info['record_storagetype'] == 'db')
			{
				if (!$info['storage_file'])
				{
					$this->logError($id, 'No RECORD file data provided');
					return false;
				}
				
				if ( in_array($info['record_type'], array('ssupload','sslink')) )
				{
					$this->createFile($info['record_realname'], $info['storage_ss'], $info['record_size'], $sssave);
				}
				else
				{
					$this->createFile($info['record_realname'], $info['storage_file'], $info['record_size'], $save);
				}
			}
			...
			$info['record_mime'] = $info['record_mime'] ? $this->getLink($info['record_mime'], 'downloads_mime') : 0;
			...
			{
				$storageSave = array( 'storage_file' => $info['storage_file'], 'storage_ss' => $info['storage_ss'] );
			}
			...
			unset($info['data']);
			unset($info['ssdata']);
			unset($info['storage_id']);
			unset($info['storage_file']);
			unset($info['storage_ss']);
			unset($info['storage_thumb']);
			
			/* Finally insert */
			$this->DB->insert( 'downloads_files_records', $info );
			$inserted_id = $this->DB->getInsertId();
			
			if ($info['file_storagetype'] == 'db')
			{
				$storageSave['storage_id'] = $inserted_id;
				$this->DB->insert( 'downloads_filestorage', $storageSave );
			}
			...
			$this->addLink($inserted_id, $id, 'downloads_files_records');
		...
		{	
			...
			if (!$info['b_records'])
			{
				$this->logError($id, 'No record ids provided');
				return false;
			}
			
			/* Explode records */
			$records  = array();
			$_records = explode(',', IPSText::cleanPermString($info['b_records']));
			
			if ( is_array($_records) && count($_records) )
			{
				foreach($_records as $_bid)
				{
					$_tmp = $this->getLink($_bid, 'downloads_files_records', true);
					if ( $_tmp !== false && is_int($_tmp) )
					{
						$records[] = $_tmp;
					}
				}
			...
			if ( count($records) < 1 )
			{
				$this->logError($id, 'No valid record ids for the backup entry');
				return false;
			}
			
			$info['b_records'] = implode(',', $records);
