...
class admin_ipchat_logs_logs extends ipsCommand
{
	...
	protected function _viewLogs()
	{
		...
		{
			$_from	= intval( @strtotime( $this->request['date_from'] ) - ( $this->settings['time_offset'] * 3660 ) );
			$_to	= intval( @strtotime( $this->request['date_to'] ) - ( $this->settings['time_offset'] * 3660 ) );
		...
		foreach( array( 'b', 'i', 'u', 'url' ) as $_key )
		{
			if ( isset($_oldBbcodes[ $_key ]) )
			{
				$_newBbcodes[ $_key ] = $_oldBbcodes[ $_key ];
			}
		}
		...
		while( $r = $this->DB->fetch($outer) )
		{
			...
			$_classname			= ( $_classname == 'row1' ) ? 'row2' : 'row1';
			...
			if( strpos( $r['log_extra'], 'private=' ) === 0 )
			{
				...
				$r['_log_message']	= sprintf( $this->lang->words['private_pre'], $_user['member_id'] ? $_user['members_display_name'] : $this->lang->words['global_guestname'] . '_' . $_uid ) . $r['_log_message'];
