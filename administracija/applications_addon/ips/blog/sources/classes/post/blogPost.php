 ...
 * Postinb Library
 * Last Updated: $LastChangedDate: 2014-12-04 21:54:57 -0500 (Thu, 04 Dec 2014) $
 * </pre>
 *
...
class blogPost
{
	...
	public function __construct( ipsRegistry $registry, $noSetUp=false )
	{
		...
		if ( ! $noSetUp )
		{
	    	...
	    	if ( ! $this->registry->isClassLoaded('tags') )
	    	{
	    		...
	    		$this->registry->setClass( 'tags', classes_tags_bootstrap::run( 'blog', 'entries' ) );
	    	}
        ...
        /* Blog this flag */
        if ( $this->settings['blog_allow_bthis'] && ! empty($this->request['btapp']) && IPSLib::appIsInstalled($this->request['btapp']) )
        {
        	$this->_blogThis = true;
        }
    }
	 ...
	 * @param	int			ID 1
	 * @param	int			ID 2
	 * @return	mixed	NULL if 'blog this' is disabled, FALSE if it failed otherwise the inserted row ID
	 */
	...
	public function htmlDateField( $curr_date='' )
	{
		...
		/* GOT POST? */
		if ( isset( $this->request['entry_day'] ) )
		{
			$_defaults[0] = $this->request['entry_month'];
			$_defaults[1] = $this->request['entry_day'];
			$_defaults[2] = $this->request['entry_year'];
			
			list($_defaults[3], $_defaults[4]) = explode( ':', $this->request['entry_time'] );
		}
	...
	{	
		...
		$this->editor->setAllowHtml( ( intval($this->org_entry['entry_html_state']) AND $this->memberData['g_blog_do_html'] ) ? 1 : 0 );
		$this->editor->setIsHtml( $this->org_entry['entry_html_state'] );
	...
	public function compilePost()
	{
		...
		$this->editor->setIsHtml( $this->request['post_htmlstatus'] && $this->memberData['g_blog_do_html'] );
		$this->editor->setAllowBbcode( true );
		...
		IPSText::getTextClass('bbcode')->parse_nl2br			= $this->request['post_htmlstatus'];
		IPSText::getTextClass('bbcode')->parse_bbcode			= 1;
		IPSText::getTextClass('bbcode')->parsing_section		= 'blog_entry';
		IPSText::getTextClass('bbcode')->parsing_mgroup			= $this->memberData['member_group_id'];
		IPSText::getTextClass('bbcode')->parsing_mgroup_others	= $this->memberData['mgroup_others'];		

		/* Entry Date - If member is not allowed to publish, publish date should be set by publisher */
		if( $this->blogFunctions->allowPublish( $this->blog ) )
		{
	...
	protected function _generatePollBox( $formType )
	{
		if ( $this->can_add_poll )
		{
			...
			if ( isset($_POST['question']) AND is_array( $_POST['question'] ) and count( $_POST['question'] ) )
			{
				...
				{
					$poll_questions[intval( $id )] = IPSText::parseCleanValue( $question );
				}
			...
			if ( isset($_POST['multi']) AND is_array( $_POST['multi'] ) and count( $_POST['multi'] ) )
			{
				...
				{
					$poll_multi[ intval( $id ) ] = intval( $checked );
				}
			}			
			...
			if ( isset($_POST['choice']) AND is_array( $_POST['choice'] ) and count( $_POST['choice'] ) )
			{
				...
				{
					$poll_choices[ intval( $id ) ] = IPSText::parseCleanValue( $choice );
				}
			}
			...
			if ( $formType == 'edit' )
			{
				if ( isset( $_POST['votes'] ) && is_array( $_POST['votes'] ) and count( $_POST['votes'] ) )
				{
					...
					{
						$poll_votes[ intval( $id ) ] = IPSText::parseCleanValue( $vote );
					}
				}
			}
