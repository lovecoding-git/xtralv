...
class ssoPublicSessions extends publicSessions
{
	...
	protected function _updateMemberSession()
	{
		...
		/* (SOS32) Total Time Spent On Forums 3.0.0 */
		if( self::$data_store['member_id'] AND self::$data_store['last_click'] > 0 )
		{
			$time_spent = time() - self::$data_store['last_click'];
			$this->DB->update( 'profile_portal', 'time_spent=time_spent+'.intval($time_spent), "pp_member_id=".intval(self::$data_store['member_id']), true, true );
	
			self::$data_store['last_click'] = 0;
		}
		/* (SOS32) Total Time Spent On Forums 3.0.0 */
	...
	public function getSession( $session_id="" )
	{
		...
		$result		= array();
		$session_id	= substr( IPSText::alphanumericalClean( $session_id, '_=' ), 0, 32 );
	...
	public function updateSession( $sessionID, $memberID, $data )
	{
		...
		$sessionID = substr( IPSText::alphanumericalClean( $sessionID, '_=' ), 0, 32 );
		$_data     = array();
		$_ignore   = array( 'id',
							'member_id',
							'ip_address',
