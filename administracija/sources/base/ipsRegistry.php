 ...
 * Last Updated: $Date: 2015-03-09 15:48:19 -0400 (Mon, 09 Mar 2015) $
 * </pre>
 *
...
class ipsRegistry
{
	...
	static public function init()
	{
		...
		if ( !empty( $_GET['setlanguage'] ) AND $_GET['langid'] AND ipsRegistry::$request['k'] )
		{
			$url = preg_replace( "/k=(.+?){32}&/", "", ( strtolower($_SERVER['HTTPS']) == 'on' ? "https://" : "http://" ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] );
			self::instance()->getClass('output')->silentRedirect( $url );
		}
		...
		if ( IPS_AREA == 'admin' )
		{
			...
			if ( ( ipsRegistry::$request['module'] != 'login' ) AND ( ! $validationStatus ) )
			{
				...
				/* @link http://community.invisionpower.com/resources/bugs.html/_/ip-board/https-for-logins-r41336 */
		...
		else if ( IPS_AREA == 'public' )
		{
			...
			ipsRegistry::$settings['noCacheKey'] = md5('$Rev: 12609 $');
	...
	protected static function _fUrlInit()
	{
		...
		if ( ipsRegistry::$settings['use_friendly_urls'] )
		{
			...
			{
				$templates = array();
				include( FURL_CACHE_PATH );/*noLibHook*/
	...
	protected function setUpSettings()
	{
		...
		ipsRegistry::$settings['fb_req_perms']   = 'email,publish_actions,user_status';
		...
		{
			define( 'IPS_URL_SHORTEN_SERVICE', 'bitly' );
		}
	}
...
class ips_DBRegistry
{
	...
	/**
	 * Reset log file paths in case they've changed
	 *
	 * @return void
	 */
	static public function resetLogPaths()
	{
		# Error log
		self::getDB()->obj['error_log']				= IPS_CACHE_PATH . 'cache/sql_error_log_'.date('m_d_y').'.cgi';
		
		# Debug log - Don't use this on a production board!
		self::getDB()->obj['debug_log']				= IPS_CACHE_PATH . 'cache/sql_debug_log_'.date('m_d_y').'.cgi';
		
		# Bad log - Don't use this on a production board!
		self::getDB()->obj['bad_log']				= IPS_CACHE_PATH . 'cache/sql_bad_log_'.date('m_d_y').'.cgi';
		
		# Slow log - Don't use this on a production board!
		self::getDB()->obj['slow_log']				= IPS_CACHE_PATH . 'cache/sql_slow_log_'.date('m_d_y').'.cgi';
	}
	...
	static public function setDB( $db_driver, $key='', $settings=array() )
	{
		...
		# Error log
		self::$dbObjects[ $key ]->obj['error_log']				= IPS_TEMP_CACHE_PATH . 'cache/sql_error_log_'.date('m_d_y').'.cgi';
		self::$dbObjects[ $key ]->obj['use_error_log']			= IN_DEV ? 0 : 1;
		
		# Debug log - Don't use this on a production board!
		self::$dbObjects[ $key ]->obj['debug_log']				= IPS_TEMP_CACHE_PATH . 'cache/sql_debug_log_'.date('m_d_y').'.cgi';
		self::$dbObjects[ $key ]->obj['use_debug_log']			= ( defined('IPS_SQL_DEBUG_LOG') AND IPS_SQL_DEBUG_LOG ) ? 1 : 0;
		
		# Bad log - Don't use this on a production board!
		self::$dbObjects[ $key ]->obj['bad_log']				= IPS_TEMP_CACHE_PATH . 'cache/sql_bad_log_'.date('m_d_y').'.cgi';
		self::$dbObjects[ $key ]->obj['use_bad_log']			= ( defined( 'IPS_SQL_FIND_EVIL_MODE' ) AND IPS_SQL_FIND_EVIL_MODE ) ? TRUE : FALSE;
		
		# Slow log - Don't use this on a production board!
		self::$dbObjects[ $key ]->obj['slow_log']				= IPS_TEMP_CACHE_PATH . 'cache/sql_slow_log_'.date('m_d_y').'.cgi';
...
class ips_CacheRegistry
{
	...
	private function init()
	{
		if ( self::$initiated !== TRUE )
		{
			...
			{
				$GLOBAL_CACHES = array();
				include( GLOBAL_CACHE_PATH );/*noLibHook*/
	...
	protected function cacheSet( $key, $val )
	{
		...
		if ( $key )
		{
			...
			if ( is_object( self::$cacheLib ) )
			{
				if ( ! $val )
				{
					/* if we're using an alternative cache - storing EMPTY for a cache that expects to always contain and array can cause foreach errors when accessed */
					if ( ! empty( $this->save_options['array'] ) )
					{
						$val = serialize( array() );
					}
